...
class AffordanceDetection():
  ...
  def train(self, images, affs, text_prompts):

    dset = AffDataset(img=images, aff_mask=affs, txt_prompt=text_prompts, processor=self.processor)
    validation_split = .2
...
def main(data_folder, bbox_eps, checkpoint_file, visualization_active, batchsize, model_folder, model_name, num_epochs, out_images, testing, test_single, img_path, prompt, image_name):
  ...
  if not test_single:
    ...
    # Training
    if not testing:
      haff_model.train(images, affs, text_prompts)
    else:
