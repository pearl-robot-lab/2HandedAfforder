        ...
        **kwargs,
        ...
        for i in range(len(pred_embeddings)):
            ...
            pred_mask_left = self.model.visual_model.postprocess_masks(
                ...
                original_size=label_list[i]['left'].shape,
            )
            ...
            pred_mask_right = self.model.visual_model.postprocess_masks(
                low_res_masks_right,
                input_size=resize_list[i],
                original_size=label_list[i]['right'].shape,
            )
        ...
        #print("SAMPLE GT MASK: ", torch.max(gt_masks_left[0]))
        #print("SAMPLE GT MASK RIGHT: ", torch.max(gt_masks_right[0]))
        #print("Weight Decoder Left: ", weight_decoder_left)
        #print("Weight Decoder Right: ", weight_decoder_right)
        #print("Weight Decoder Both: ", weight_decoder_both)
        #print("SHAPES: ", weight_decoder_left.size(), weight_decoder_both.size(), pred_masks_left.size())
        #print("Sum Pred Left Before: ", torch.sum(pred_masks_left))
        #print("Sum Pred Right Before: ", torch.sum(pred_masks_right))
        #print("Sum Pred Left Max: ", torch.max(pred_masks_right))
        ...
        #print("Sum Pred Left After: ", torch.sum(pred_masks_left))
        #print("Sum Pred Right After: ", torch.sum(pred_masks_right))
        #print("Sample Pred Mask: ", pred_masks_left[0])
        for batch_idx in range(len(pred_masks_left)):
            ...
            mask_bce_loss_left += (
                sigmoid_ce_loss(pred_mask_left, gt_mask_left, num_masks=gt_mask_left.shape[0])
                * gt_mask_left.shape[0]
            )
            mask_dice_loss_left += (
                dice_loss(pred_mask_left, gt_mask_left, num_masks=gt_mask_left.shape[0])
                * gt_mask_left.shape[0]
            )

            mask_bce_loss_right += (
                sigmoid_ce_loss(pred_mask_right, gt_mask_right, num_masks=gt_mask_right.shape[0])
                * gt_mask_right.shape[0]
            )
            mask_dice_loss_right += (
                dice_loss(pred_mask_right, gt_mask_right, num_masks=gt_mask_right.shape[0])
                * gt_mask_right.shape[0]
            )
        ...
        taxonomy_ce_loss = taxonomy_ce_loss / len(pred_masks_left)
